            ...
            <ul className="flex space-x-6 px-4 py-3">
                {produkter.map((category, index => (
                    <li key={category.name} className="relative">
                        <button onClick={() => toggleDropDown(index)} className="flex items-center gap-1 focus:outline-none">
                        ...
                        {openIndex === index && (
                            <ul className="absolute left-0 mt-2 bg-white text-black rounded shadow z-10 min-w-[180px]">
                                {category.subcategories.map((sub) => (
                                    <li key={sub.name}> <Link to={`/produkter/${sub.name}`} className="block px-4 py-2 hover:bg-gray-100" >
                                        {sub.name}
                                        </Link>
                                        </li>
                                ))}
